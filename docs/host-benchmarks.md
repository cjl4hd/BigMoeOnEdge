...
Each model's raw cell table is collapsed under its own `<details>` block below the section
heading — open it for the engine commits and fault/hit columns; the narrative bullets
carry the interpretation. The runnable flag sets derived from these rows live in
...
number with. Lossless and lossy are separate tables on purpose — a lossy row is a speed/quality
trade and is only quotable with its quality evidence next to it
([benchmark-method.md](benchmark-method.md) §knob groups). Full mechanism columns per cell:
[§Feature A/B](#feature-ab-the-computelatency-hiding-layer-2026-09-20) below and the per-model
sections; each feature's own doc carries its dedicated protocol.
