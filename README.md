...
| **`--rs-seq N`** — recurrent-state snapshot rollback (experimental, off by default) | Partial `seq_rm` rewinds position but not recurrent cell state on hybrids | Restore proven exact (9/9 cutsweep cells) and live on two 35B hybrids: divergence turn 33 prompt / 203 reused, prefill 42.6→19.3 s on the Cyber-Tiel batch; warmup composes with it (first turn 1 fresh / 203 restored) — off by default pending an upstream release | Recurrent hybrids fall back to full re-prefill on any rewind | `feat/session-residency`; upstream #29085, fix committed `7b2ec36d1` (PR #29117 closed pending #29085) |
